...
export function SkeletonForm({ fields = 4, className }: SkeletonFormProps) {
  return (
    <div className={cn("space-y-4 rounded-lg border border-slate-200 bg-white p-6", className)}>
      {Array.from({ length: fields }).map((_, i) => (
        <div key={i}>
          <div className="mb-1 h-3 w-20 animate-pulse rounded bg-slate-200" />
          <div className="h-10 animate-pulse rounded border border-slate-200 bg-slate-50" />
        </div>
      ))}
