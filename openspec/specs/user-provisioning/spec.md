...
Provision a per-user Turso database at web registration (mirroring the desktop flow) so web-registered users get a `user_databases` row, working dashboard access via `getUserDb`, and a JWT carrying `dbUrl`/`dbToken`/`academyName`. The shared DB remains the source of truth for subscriptions and payments.
